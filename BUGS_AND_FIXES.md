...
## Engineering upgrade 24: international privacy frameworks reference (minor suggestion #4, final item)

**Status: DONE, 2026-09-11.** Legal-research support, not an engineering change — no code accompanies this. The last of the six minor-suggestions from the manuscript's accepted-with-revisions review; not originally prioritized in this session's "all three, in that order" plan (detection tradeoffs, taxonomy reference, adversarial evasion), closed out after the user asked to finish everything remaining.

`INTERNATIONAL_PRIVACY_FRAMEWORKS.md` surveys six jurisdictions (CCPA/CPRA, China's PIPL, Brazil's LGPD, India's DPDP Act, Singapore's PDPA, EU AI Act logging provisions) chosen for architectural relevance to REDACT's own pseudonymize/tokenize/redact taxonomy, each researched via live web search rather than relied on from training-data memory (regulatory status changes; several of these frameworks moved substantively in 2026 — China's cross-border three-pathway system completed Jan 1 2026, India's DPDP Rules notified Nov 2025, EU AI Act high-risk provisions enforceable Aug 2026). Explicit "not legal advice" disclaimer, and explicitly not exhaustive of the ~140 national privacy laws now in force.

**Real, load-bearing finding: Singapore's PDPA diverges from the EU/Brazil/China consensus on pseudonymization.** GDPR, LGPD, and PIPL all treat pseudonymized data as remaining personal data regardless of technical strength; PDPA treats successful pseudonymization as capable of exiting scope entirely if it genuinely prevents identification. Reasoned against REDACT's own already-measured evidence rather than left abstract: `validation/investigative_utility/`'s existing 100% candidate-list re-identification result for pseudonymization (30/30 trials, given the key and a short candidate list) suggests REDACT's current implementation likely would not clear Singapore's more permissive bar either, in practice — an inference from an existing result, explicitly flagged as such, not a new experiment run against PDPA's specific legal test.

**Devil's-advocate section ties directly into this session's own earlier live work, not generic caveats:** the EU-US Data Privacy Framework — the adequacy mechanism the real Azure Blob/Splunk integrations built and confirmed live earlier this session (Engineering upgrades 21-22) would lean on for EU-sourced telemetry — is under a live, current legal challenge as of June/July 2026 (Schrems' argument to the European Commission following the US Supreme Court's *Trump v. Slaughter* ruling, plus a pending CJEU appeal), disclosed as a genuine present-day instability rather than a hypothetical risk. A favorable structural finding is also noted: CCPA/CPRA's definition of qualifying pseudonymous data (separated from identifying information, with controls preventing recombination) closely matches `TokenStore`'s existing forward/reverse-map separation design, which predates this document.

`ROADMAP.md` item 18 added with the same findings. **Status of all six minor revision suggestions, stated precisely rather than rounded up to "all done":** taxonomy/transformation detail (fully addressed, `TAXONOMY_AND_TRANSFORMATION_LOGIC.md`), investigative-utility tradeoffs (fully addressed, `validation/investigative_utility/`), detection performance tradeoffs (fully addressed, `DETECTION_PERFORMANCE_TRADEOFFS.md`), international privacy frameworks (fully addressed, this entry), future cloud-native SIEM/AI-SOC implications (addressed via the real Splunk/Azure Blob integrations, `ROADMAP.md` item 16, plus this document's own framing of where telemetry infrastructure and privacy law now intersect) — five of six fully closed. **The sixth, adversarial AI/model drift/future AI-assisted telemetry analysis, is only PARTIALLY addressed**: its concrete, measurable sub-component (adversarial evasion against REDACT's own detection layers) was built and run live (Engineering upgrade 23); the broader narrative discussion the original suggestion also named — general adversarial-AI literature, model drift as a topic, speculative future AI-assisted analysis challenges — was assessed earlier this session as a writing/literature-synthesis task rather than an engineering-evidence one and was explicitly not built here. That remainder is real, open, and belongs in whichever session handles prose/citation work for this chapter, not silently implied as finished by this entry.

## Engineering upgrade 25: adversarial AI / model drift / future AI-SOC narrative (minor suggestion #1, final piece, all six suggestions now closed)

**Status: DONE, 2026-09-11.** Literature-research/synthesis support, not an engineering change. Completes minor suggestion #1's remaining scope after "Engineering upgrade 23" (adversarial evasion-testing) closed its concrete, measurable sub-component but explicitly left the broader narrative discussion open (see Engineering upgrade 24's own precise accounting of this).

`ADVERSARIAL_AI_MODEL_DRIFT_AND_FUTURE_AI_SOC.md` researched current sources (2024-2026, several from within months of this writing) rather than working from prior knowledge, and deliberately grounds every section in REDACT's own already-measured results reframed under a new lens, rather than treating this as free-floating commentary disconnected from the actual codebase.

**Real, non-obvious connection found, not assumed:** "Poisoning the Watchtower" (Pandey & Bhujang, arXiv:2605.24421, May 2026) studies prompt-injection attacks against LLM-augmented SOC analyst tools via attacker-controlled log fields, and names usernames specifically as one of the exact fields carrying this risk -- the identical field category (PERSON) this project's own Layer 4 flattened-name detector already discloses as its single most significant measured gap. Checked the paper's actual abstract directly (via arXiv, not a secondhand summary) before citing its numbers: 96% context-manipulation injection success on summarization tasks without defenses, falling to 38% even under constrained output; persona-hijack attacks suppress 68% of malicious logs under a naive classifier. This means an unredacted or evasion-defeated username isn't just a REDACT-scope privacy exposure -- if that log line later reaches an LLM-based SOC analyst, it's potentially a prompt-injection vector too, a distinct risk this project doesn't claim to address but can now cite concretely.

**Model drift section makes a careful, load-bearing distinction rather than conflating two different things:** `drift.py`'s real, shipped field-level taxonomy drift detector (data drift -- does a field's PII-carrying rate change over time) is explicitly NOT the same as classical model/concept drift (REDACT's detection ensemble is never retrained, so there's no model-weights drift to monitor). Three already-published REDACT results were reframed under a drift lens for the first time, not re-measured: the flattened-name layer's population-shift collapse (50.3% Faker-matched -> 1.4% disjoint-locale -> 15.2% real-frequency-weighted), PIIBench's domain-shift gap (P=0.354/R=0.650 out-of-domain), and the real-vs-synthetic Loghub precision gap as format-shift evidence. A real, honestly-named gap: none of this is wired into `drift.py`'s own monitoring today, which is disclosed as an open item, not glossed over.

**Devil's-advocate synthesis across all three sections:** "tokenize PII before LLM calls" (a real, current best-practice recommendation in SOC-copilot security guidance) is only as strong as the detection layer performing that tokenization -- and this project has now measured, concretely and repeatedly across this whole session's work, exactly where that layer's coverage ends.

`ROADMAP.md` item 19 added with the same findings. **This closes all six of the manuscript's minor revision suggestions in full** -- taxonomy/transformation detail, investigative-utility tradeoffs, detection performance tradeoffs, international privacy frameworks, future cloud-native SIEM/AI-SOC implications, and now adversarial AI/model drift/future AI-assisted telemetry analysis. Nothing from the original six-item list remains open.
