...
**Not yet run:** needs the user's own machine, same as every other real-network fetch in this project. Next step: `cd validation/real_data && bash download_loghub.sh && python3 inject_and_evaluate.py` -- the OpenStack/Zookeeper IP-recall numbers this produces would be a genuinely new, previously-never-executed data point for the paper, not a rerun of something already reported.
