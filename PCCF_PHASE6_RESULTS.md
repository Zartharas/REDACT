# PCCF phase 6 results: the 4 phase-5 floor misses (split shift vs chance)

Pre-registration: `PCCF_PHASE6_PREREGISTRATION.md`, written before any phase-6
number was computed. Script: `validation/real_data/phase6/pccf_phase6_split_diagnostic.py`.
It reuses the phase-5 caches unchanged and reproduces phase 5 exactly: ru
{ner} 0.859 and it {ner} 0.931. It ran 50 seeds × 2 split protocols × 18 rows
× 2 scorers.

## Verdicts (mechanical)
- **H25 SUPPORTED.** Under exchangeable re-splits (E1), LR-UD's mean recall
  meets the floor in every eligible (language, group), and so does the
  rule-scorer control. **This finds no method defect.**
- **H26 SUPPORTED.** With the original split protocol (E2: calibrate on
  *train*, test on *validation*), {ner} recall sits systematically lower than
  under pooled splits:
  - it −0.023 (0.929)
  - hi −0.007 (0.944)
  - id_hf −0.008 (0.940)
  - ar_wiki −0.012 (0.938), not a phase-5 miss
- **H27 SUPPORTED.** The phase-5 tolerance ignored calibration-set randomness.
  Under exact exchangeability it is missed in **8.4 %** of 2,100 checks, not
  the nominal 2.3 %. The combined-variance model predicts ≤ 12.5 %; the
  observed rate is lower because split-conformal is slightly conservative.

## What the 4 misses were
| lang | phase-5 {ner} recall | explanation |
|---|---|---|
| it | 0.931 | **split shift.** The pooled mean is 0.951; the original-protocol mean is 0.929. |
| hi | 0.932 | **split shift** (small): 0.951 → 0.944 |
| id_hf | 0.930 | **split shift** (small): 0.948 → 0.940 |
| ru | 0.859 | **chance.** The single split is already exchangeable; the 50-seed mean is 0.967, and n_cal_true ≈ 35 makes single draws noisy. Even under the combined-variance floor (0.860), it is a tail draw (about 2 SD). |

## Consequences
1. **The guarantee holds when calibration and deployment data are
   exchangeable.** When they are not, coverage drifts in **either direction**:
   id +0.016, de +0.009, it −0.023. So the shift is unpredictable, and the
   operational fix is to calibrate on data drawn from the deployment stream,
   which is the phase-3 lesson, now quantified.
2. **Forward-only rule (pre-committed):** future phases judge floors with
   `0.95 − 2·sqrt(α(1−α)(1/n_cal_true + 1/n_test_true))`. Phase-5 verdicts are
   **not** re-scored. As a sensitivity check only: it, hi and id_hf would pass
   under it, and ru would still miss by 0.001.
3. **Caveats (devil's advocate):**
   - Exchangeability holds at the document level, while candidates within a
     document are correlated. The E1 means of 0.948–0.950 (id_hf, ar_wiki,
     fr, de, tr) sit right at the target, not above it.
   - With ~45 group checks per sweep, reporting "floors hold in all
     languages" as a yes/no verdict over-punishes chance. Future
     pre-registrations should state the expected number of chance misses, or
     use a multiplicity-aware criterion.
...
| in | LR-UD | {ner} | 282 / 565 | 0.951 | 0.013 | 0.946 | ok |
...
| nl | yes | 0.953 ± 0.002 | 0.949 ± 0.001 | -0.003 | no |
