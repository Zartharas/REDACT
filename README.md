# PII/PHI detection pipeline — reproducible proof of concept

This is the working, executable core referenced in the IntechOpen chapter. It generates a synthetic, ground-truth-annotated security log dataset, runs a three-layer detection ensemble (regex, Presidio NER, Shannon entropy) against it, and reports measured precision/recall/F1 and throughput. No real data, no paid tools, no Docker required.
