...
**3. The entropy fallback, as commonly described in the literature, does not pull its weight on structured log fields.** A threshold sweep (min length 12–20 chars, entropy threshold 3.3–4.2) found no operating point that gets meaningful unique recall without a high false-alarm rate: at threshold 3.3, it flags 34.8% of clean (non-PII) lines, and even at that permissive setting it only catches 142 of 6,199 gold PII spans (2.3%) that regex and NER both missed, mostly redundant re-detection of things already caught. The false alarms are dominated by fixed-vocabulary structured tokens like `EventID=4672`, which have moderate character diversity relative to length but carry no PII at all.
   ...
   **Tested against its actual intended use case (2026-08-07, ROADMAP item 11):** `validation/entropy_fair_test/` builds a dedicated 2,000-entry corpus of realistic log lines carrying genuinely secret-shaped tokens (JWT-style bearer tokens, AWS-style access/secret key pairs, GitHub-style PATs, session cookies, SHA-256 hashes) alongside clean lines including UUIDs as a deliberately hard negative case (structurally lower entropy per character than a true secret, despite looking random at a glance). Result, sweeping the same threshold range as above: **best F1 of 0.811** (precision 0.811, recall 0.812, 9.7% false-alarm rate) at `min_len=12, threshold=4.2` — a large gap from the 2.3%-recall/34.8%-false-alarm number above, confirming that number was an honest measurement of the wrong target, not a verdict on the technique. The remaining false positives at that operating point are concentrated almost entirely in UUIDs embedded in URLs/query parameters, confirmed by direct inspection — a genuine, still-open source, where a UUID-shape exclusion would likely help further but hasn't been built or measured. See `validation/entropy_fair_test/README.md` for the full sweep and what this test doesn't establish (other secret formats, real production log volume).
